...
The goal of local testing is a throwaway `ephys` database on `localhost` that you
...
| Web dashboard (`web/ephys-dashboard.html`) | Renders from in-page synthetic data | No |
...
Use a **test-only database name** (`ephys_test` below) so you never risk running
these destructive steps against a real `ephys` database. Point MATLAB and the GUI
at `ephys_test` for the whole exercise.
...
```powershell
dropdb -U postgres --if-exists ephys_test          # clean slate
createdb -U postgres ephys_test
psql -U postgres -d ephys_test -v ON_ERROR_STOP=1 -f design_docs/schema.sql
```
...
```powershell
psql -U postgres -d ephys_test -c "\dt ephys.*"      # tables
psql -U postgres -d ephys_test -c "\dv ephys.*"      # views (expect *_active)
psql -U postgres -d ephys_test -c "\df ephys.*"      # functions (fn_artifact_lineage, fn_forbid_mutation)
```
...
```powershell
psql -U postgres -d ephys_test -v ON_ERROR_STOP=0 -c @'
INSERT INTO ephys.person (full_name, email, role)
VALUES ('Test User', 'test@local', 'PI');
-- Both of the next two statements MUST fail with the mutation-forbidden error:
UPDATE ephys.person SET role = 'tech' WHERE email = 'test@local';
DELETE FROM ephys.person WHERE email = 'test@local';
'@
```
...
```powershell
psql -U postgres -d ephys_test -c "CREATE ROLE ephys_rw LOGIN PASSWORD 'test-pw';"
psql -U postgres -d ephys_test -c "GRANT USAGE ON SCHEMA ephys TO ephys_rw;"
psql -U postgres -d ephys_test -c "GRANT SELECT, INSERT ON ALL TABLES IN SCHEMA ephys TO ephys_rw;"
psql -U postgres -d ephys_test -c "GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA ephys TO ephys_rw;"
```
...
point at `localhost` / `ephys_test` / `ephys_rw`. It exercises the full path the
...
```matlab
n = db.runQuery("SELECT count(*) AS n FROM ephys.event_active;");   % works
% This must error (syntactic guard and/or READ ONLY transaction):
try
    db.runReadOnlyQuery("INSERT INTO ephys.person(full_name,email) VALUES('x','y@z');");
...
With the class folders already on the path and `ephys_test` populated by §2:
...
Log in against `localhost` / `ephys_test` / `ephys_rw`, then verify each surface
...
```powershell
python -m http.server 8777 --directory web    # matches the ephys-web launch config
```

Open <http://localhost:8777/ephys-dashboard.html> (or just double-click the file;
an internet connection is needed once to fetch Chart.js from its CDN).

**Pass:** the page renders all charts from `window.EPHYS_DATA`, the browser
...
Run top to bottom against `ephys_test` before you deploy to a server:

- [ ] `schema.sql` loads clean with `ON_ERROR_STOP=1`.
- [ ] Schema objects present (`\dt`, `\dv`, `\df` in the `ephys` schema).
- [ ] `UPDATE`/`DELETE` on an immutable row are rejected by `fn_forbid_mutation()`.
- [ ] `ephys_rw` role can `SELECT`/`INSERT`, cannot `UPDATE`.
- [ ] MATLAB `ver` / `Database_Toolbox` / `exist('postgresql')` all check out.
- [ ] `CarasLabDB(...)` connects to `localhost`/`ephys_test` and `isOpen()`.
...
- [ ] `ephys-dashboard.html` renders with no console errors.
...
```powershell
dropdb -U postgres --if-exists ephys_test
```

Because everything ran against `ephys_test` on `localhost`, dropping that database
removes all test data. Nothing you did here touches a production `ephys` database.
...
1. Create the **real** `ephys` database instead of `ephys_test`, and (for a shared
...
2. Point MATLAB and the GUI at the server hostname/IP instead of `localhost`, and
   use the production `ephys_rw` password.
